...
all.ids <- c(levels(Idents(D7_20220307_Aug22Revisions)), levels(Idents(tyser)))
pal <- DiscretePalette(length(all.ids))
names(pal) <- all.ids

# *** Do the transfer WITH integration: *******************
combined <- TransferAnnotations(tyser, D7_20220307_Aug22Revisions,
                                refdata,
                                c("tyser", "D7"), "AllCells", TRUE)
