...
all.ids <- c(levels(Idents(D4_merged)), levels(Idents(tyser)))
pal <- DiscretePalette(length(all.ids))
names(pal) <- all.ids

# *** Do the transfer WITH integration: *******************
combined <- TransferAnnotations(tyser, D4_merged,
                                refdata,
                                c("tyser", "D4 merged"), "AllCells", TRUE)
