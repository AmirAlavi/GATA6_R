...
all.ids <- c(levels(Idents(D4_SB431542_Aug22Revisions)), levels(Idents(tyser)))
pal <- DiscretePalette(length(all.ids))
names(pal) <- all.ids

# *** Do the transfer WITH integration: *******************
combined <- TransferAnnotations(tyser, D4_SB431542_Aug22Revisions,
                                refdata,
                                c("tyser", "D4_SB431542"), "AllCells", TRUE)
...
patchwork <- patchwork + plot_annotation(
  ...
  caption = "We used the Tyser Human Gastrula scRNA-seq count data (1195 cells) as the labeled reference dataset, and used our D4(SB431542) cells (2534) as a query dataset.\nThen we used Seurat's FindTransferAnchors and TransferData functions to annotate the query dataset with Cell Type and Age labels from the reference.",
  tag_levels = 'A'
) & theme(plot.tag = element_text(size = 10))
ggsave(snakemake@output[[1]], width = 24, height = 6, units = "in")

# *** Create classification proportion bar plot
plot.data <- combined@meta.data[combined$orig.ident == "10X_D4_SB431542_Aug22Revisions", ]
