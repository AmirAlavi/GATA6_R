...
all.ids <- c(levels(Idents(D5_20220330_Aug22Revisions)), levels(Idents(tyser)))
pal <- DiscretePalette(length(all.ids))
names(pal) <- all.ids

# *** Do the transfer WITH integration: *******************
combined <- TransferAnnotations(tyser, D5_20220330_Aug22Revisions,
                                refdata,
                                c("tyser", "D5"), "AllCells", TRUE)
