...
load_hu_gas_markers <- function() {
  markers <- readRDS("data/Human_Gastrula/hu_gas_markers.RDS")
  n_clusters <- length(unique(markers$cluster))
  markers_list <- vector(mode = "list", length = n_clusters)
  names(markers_list) <- unique(markers$cluster)
  for (i in 1:length(markers_list)) {
...
# Human Gastrula
hu_gas <- readRDS("data/Human_Gastrula/hu_gas_seurat_object_processed.RDS")
hu_gas_all_genes <- row.names(hu_gas)
background <- intersect(GATA6_all_genes, hu_gas_all_genes)

hu_gas_markers <- load_hu_gas_markers()

g <- compareMarkersBetweenDatasets(GATA6_markers, hu_gas_markers, background, "GATA6_cluster", "Human_Gastrula_cluster")
g + labs(x = "Human Gastrula cluster", y = "-log10(Hyperg Pval)")
...
# Human Gastrula and Xiang et al. together
background <- Reduce(intersect, list(GATA6_all_genes, xiang_all_genes, hu_gas_all_genes))
both_markers <- c(xiang_markers, hu_gas_markers)
g <- compareMarkersBetweenDatasets(GATA6_markers, both_markers, background, "GATA6_cluster", "Xiang_hu_gas_cluster")
g + labs(x = "Xiang et al./Hu. Gas. cluster", y = "-log10(Hyperg Pval)")
